...
def run_grid_search_experiments():
    ...
    models_config = {
        "Linear Regression": {"estimator": LinearRegression(), "grid": {}},
        "Ridge": {
        ...
        "Random Forest": {
            ...
            "grid": {"model__n_estimators": [100, 200], "model__max_depth": [6, 10, None]},
        },
        "XGBoost": {
    ...
    for encoding in ["onehot", "target"]:
        ...
        for name, config in models_config.items():
            ...
            full_pipeline = Pipeline(
                steps=[("preprocessor", preprocessor), ("model", config["estimator"])]
            )
            ...
            with mlflow.start_run(run_name=run_name) as run:
                ...
                print(
                    f"Results: R² = {metrics['r2']:.4f}, RMSE = {metrics['rmse']:.4f}, MAE = {metrics['mae']:.4f}"
                )
    ...
    with open(comparison_md_path, "w") as f:
        ...
        f.write(
            f"The chosen final model is **{best_model_name}** with **{best_encoding} encoding** (R² = {best_r2:.4f}), saved to `models/model_pipeline.joblib`.\n"
        )
