...
def run_model_explanations():
    ...
    print(
        f"Instance 2 (High Price): Index={high_idx}, Predicted Price={predictions[high_idx]:.2f} Lakhs (vs Mean={mean_pred:.2f} Lakhs)"
    )
    ...
    plt.title(
        f"Price Deconstruction: Low Price Example (Predicted: {predictions[low_idx]:.2f} Lakhs)",
        fontsize=14,
    )
    plt.tight_layout()
    ...
    plt.title(
        f"Price Deconstruction: High Price Example (Predicted: {predictions[high_idx]:.2f} Lakhs)",
        fontsize=14,
    )
    plt.tight_layout()
    ...
    print(
        f"Listing 1 (Actual price: {y_test.iloc[low_idx]} Lakhs, Predicted: {predictions[low_idx]:.2f} Lakhs):"
    )
    print(
        f"- Location: {low_features['location']}, Sqft: {low_features['total_sqft']}, BHK: {low_features['bhk']}, Bath: {low_features['bath']}"
    )
    print(
        f"- Interpretation: This property was priced below average primarily because of its smaller size ({low_features['total_sqft']} sqft) and fewer bedrooms ({low_features['bhk']} BHK)."
    )
    ...
    print(
        f"- Location: {high_features['location']}, Sqft: {high_features['total_sqft']}, BHK: {high_features['bhk']}, Bath: {high_features['bath']}"
    )
    print(
        f"- Interpretation: This property was priced significantly above average due to its large square footage ({high_features['total_sqft']} sqft), high number of bathrooms ({high_features['bath']}), and premium location."
    )
